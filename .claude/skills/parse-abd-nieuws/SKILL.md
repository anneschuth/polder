...
- `role` (string): functie zoals "Afdelingshoofd Beleid Wonen, Directie Wonen, ministerie van VRO". **Casing**: begin de `role` met een hoofdletter (de functietitel: `Afdelingshoofd`, `Directeur`, `Directeur-generaal`, `Secretaris-generaal`, `Minister`, `Staatssecretaris`, ...) ook als het bericht klein schrijft. Uitzondering: gevestigde afkortingsvormen die met een kleine letter beginnen (`pSG`, `plv.`). Dit raakt alleen het afgeleide `role`-veld; een letterlijke `evidence_snippet` behoudt de bron-casing (quote-or-die).
